...
app.post('/projects', verifyToken, (req, res) => {
  ...
  getIdByEmail(req.user.useremail, (err, userID) => {
    ...
    db.beginTransaction(err => {
      ...
      const projectQuery = `INSERT INTO projects (projectname, description, startdate, enddate, createdby, status) VALUES (?, ?, ?, ?, ?, 'Not Started')`;
        ...
        const milestonePromises = projectData.milestones.map(milestone => {
          return new Promise((resolve, reject) => {
            ...
            db.query(milestoneQuery, [projectId, milestone.name, milestone.description, milestone.startDate, milestone.endDate], (err, result) => {
              ...
              console.log('MiletstoneID:',milestoneId);
              const taskPromises = (projectData.tasks[milestone.name] || []).map(task => {
                return new Promise((resolve, reject) => {
                  getIdByEmail(task.assignedTo, (err, assignedUserId) => {
                    if (err) return reject(err);

                    const taskQuery = `INSERT INTO tasks (taskid, milestoneid, seq, taskname, description, assignedto, startdate, enddate, status) VALUES (NULL, ?, NULL, ?, NULL, ?, ?, ?, 'Not Started')`;
                    db.query(taskQuery, [milestoneId, task.name, assignedUserId, task.startDate, task.endDate], (err, result) => {
